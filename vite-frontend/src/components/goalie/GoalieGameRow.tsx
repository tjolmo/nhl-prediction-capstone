    ...
    >
      ...
      <td className="py-3 px-4">
        <span className="text-sm font-bold text-slate-700 bg-slate-100 px-2 py-0.5 rounded-md">
          {game_stats.opposing_team_tricode}
        </span>
      </td>
      <td className="py-3 px-4 text-center">
        <span
          className={`inline-flex items-center justify-center w-8 h-7 rounded-full text-sm font-black ${
            game_stats.saves >= 30 ? "bg-emerald-500 text-white" : "bg-slate-100 text-slate-600"
            }`}
        >
          {game_stats.saves}
        </span>
      ...
      <td className="py-3 px-4 text-center">
        <span
          className={`text-sm font-bold ${
            game_stats.goals_against === 0 ? "text-emerald-600" : game_stats.goals_against >= 3 ? "text-red-500" : "text-slate-500"
          }`}
        >
